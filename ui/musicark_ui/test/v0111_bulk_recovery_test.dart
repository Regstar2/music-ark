...
void main() {
  ...
  testWidgets('scope context keeps collection and long local folder visible on narrow layout', (
    tester,
  ) async {
    const path = r'D:\Music\A very very very long library root that must not break a narrow layout';
    await pumpShell(
      tester,
      const Padding(
  ...
  testWidgets('bulk dialog defaults to managed uploaded role and requires rights', (
    tester,
  ) async {
    ...
    expect(find.byKey(const Key('yandex-batch-upload-dialog')), findsOneWidget);
    expect(find.text('Загрузить 2 треков в Яндекс Музыку'), findsOneWidget);
    expect(
      tester.widget<FilledButton>(find.byKey(const Key('yandex-batch-submit'))).onPressed,
      isNull,
    );
  ...
  testWidgets('delivery unknown is manual-check only and never offered as blind retry', (
    tester,
  ) async {
    final targetBridge = FakeYandexUploadBridge(
      playlists: const [
    ...
    expect(find.byKey(const Key('yandex-batch-manual-check')), findsOneWidget);
    expect(find.byKey(const Key('yandex-batch-retry-failures')), findsNothing);
