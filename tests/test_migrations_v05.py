        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with closing(sqlite3.connect(db)) as conn:
                with conn:
                    ...
                    conn.execute("INSERT INTO app_metadata(key, value) VALUES('schema_version', '0.1.0')")
                    for version, steps in MIGRATION_STEPS:
                        if version > "1.3.0":
                            continue
                        for step in steps:
                            step(conn)
                        conn.execute("""INSERT INTO app_metadata(key, value) VALUES('schema_version', ?)
                                      ON CONFLICT(key) DO UPDATE SET value=excluded.value""", (version,))
                    conn.execute("""INSERT INTO provider_collection_snapshots(
                        provider_id, collection_id, account_json, item_count, refreshed_at
                    ) VALUES ('yandex_music', 'liked', '{"user":"kept"}', 1, '2026-08-14T00:00:00Z')""")
                    conn.execute("""INSERT INTO provider_collection_items(
                    ...
                    conn.execute("""INSERT INTO local_library_roots(path, normalized_path)
                                  VALUES ('C:/Music', 'c:/music')""")
                    root_id = int(conn.execute("SELECT id FROM local_library_roots").fetchone()[0])
                    conn.execute("""INSERT INTO local_audio_files(
                        ...
                        metadata_json, title, artists_json, album, availability
                              ...
                              '["Artist"]', 'Album', 'available')""", (root_id,))
            ...
            with closing(sqlite3.connect(db)) as conn:
                ...
                cache_count = conn.execute("SELECT COUNT(*) FROM provider_collection_items WHERE external_id='101'").fetchone()[0]
                local = conn.execute("""SELECT title, artists_json, normalized_title, normalized_artists_text,
                                      duration_bucket FROM local_audio_files WHERE path='C:/Music/Song.flac'""").fetchone()
