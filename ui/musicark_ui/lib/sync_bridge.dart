...
abstract interface class SyncBridgeClient {
  ...
  Future<Map<String, dynamic>> createPlan({required String scopeType, String? scopeId});
...
class SyncBridge implements SyncBridgeClient {
  ...
  @override
  Future<Map<String, dynamic>> plan(String planId) => _run('plan', planId: planId);
  ...
  Future<Map<String, dynamic>> _run(
    ...
    final args = <String>[
      ...
      if (scopeType != null && scopeType.isNotEmpty) ...['--scope-type', scopeType],
      if (scopeId != null && scopeId.isNotEmpty) ...['--scope-id', scopeId],
      if (targetPath != null && targetPath.isNotEmpty) ...['--target-path', targetPath],
  ...
  bool _looksLikeRepoRoot(Directory directory) {
    ...
    return File('${directory.path}${separator}pyproject.toml').existsSync() &&
        File('${directory.path}${separator}src${separator}musicark${separator}sync${separator}bridge.py').existsSync();
  }
...
class FakeSyncBridge implements SyncBridgeClient {
  ...
  Map<String, dynamic> samplePlan({String status = 'planned'}) => {
        ...
        'operations': [
          ...
          },
          {
            ...
            'status': 'informational',
            'metadata': {'title': 'Analyze Me', 'artists': ['Artist'], 'matchingRequired': true},
            'result': {},
          },
          {
            'id': 5,
  ...
  @override
  Future<Map<String, dynamic>> createPlan({required String scopeType, String? scopeId}) async {
    createCalls++;
  ...
  @override
  Future<Map<String, dynamic>> plan(String planId) async => currentPlan ?? samplePlan();
  ...
  Future<Map<String, dynamic>> apply(String planId, {required bool confirm}) async {
    ...
    return {'plan': currentPlan, 'result': currentPlan!['result'], 'repeated': false};
  }
