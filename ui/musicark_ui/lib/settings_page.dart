                  ...
                  Row(
                    children: [
                      ...
                      Chip(
                        ...
                        label: Text(l10n.settingsAutoSaveStatus),
                      ),
                    ],
                  ),
...
class _NetworkAccessCardState extends State<_NetworkAccessCard> {
  ...
  void _applyResult(Map<String, dynamic> result) {
    ...
    if (settings is Map) {
      final rawMode = '${settings['mode'] ?? _mode}';
      _mode = switch (rawMode) {
        'auto' || 'warp' => 'system',
        'direct' || 'custom_proxy' || 'system' => rawMode,
        _ => 'system',
      };
      _proxyScheme =
          '${settings['proxy_scheme'] ?? settings['proxyScheme'] ?? _proxyScheme}';
      _host.text =
          '${settings['proxy_host'] ?? settings['proxyHost'] ?? _host.text}';
      _port.text =
          '${settings['proxy_port'] ?? settings['proxyPort'] ?? _port.text}';
      _username.text =
          '${settings['proxy_username'] ?? settings['proxyUsername'] ?? _username.text}';
    }
  ...
  Future<void> _loadInitial() async {
    ...
    setState(() => _busy = true);
    try {
      _applyResult(await widget.bridge.getNetworkSettings());
    } catch (error) {
      _message = '$error';
    } finally {
      if (mounted) setState(() => _busy = false);
  ...
  Future<void> _testVisibleSettings() async {
    ...
    try {
      _applyResult(
        await widget.bridge.updateNetworkSettings(_visibleSettings()),
      );
  ...
  String _networkDetail(Map<String, dynamic> item, V012Strings s) {
    if (item['reachable'] == true) {
      ...
      if (status is num && status >= 400) {
        return 'HTTP $status · ${s.hostReached}';
      }
      return 'HTTP $status';
    }
  ...
  Widget build(BuildContext context) {
    ...
    final apiOk =
        _networkItems.where((item) => item['state'] == 'ok').length;
    final hostReached = _networkItems
        .where((item) => item['state'] == 'host_reached')
        .length;
    final failed = _networkItems
        .where(
          (item) =>
              item['state'] == 'failed' && item['optional'] != true,
        )
        .length;
          ...
          children: [
            ...
            SegmentedButton<String>(
              ...
              onSelectionChanged: _busy
                  ? null
                  : (value) {
                      if (value.isNotEmpty) _changeMode(value.first);
                    },
            ),
            ...
            Wrap(
              ...
              children: [
                FilledButton.tonalIcon(
                  ...
                  onPressed: _busy ? null : _saveVisibleSettings,
                  icon: const Icon(Icons.save_outlined),
                  label: Text(s.save),
                ),
                OutlinedButton.icon(
                  key: const Key('network-test'),
                  onPressed: _busy ? null : _testVisibleSettings,
            ...
            if (_message != null) ...[
              ...
              const SizedBox(height: 8),
              Text(
                s.networkSummary(apiOk, hostReached, failed),
                key: const Key('network-result'),
              ),
            ],
      ...
      child: Padding(
        ...
        child: AnimatedBuilder(
          ...
          builder: (context, _) {
            ...
            final title = signedIn && session.displayName.isNotEmpty
                ? session.displayName
                : l10n.settingsYandex;
            ...
            return Row(
              children: [
                CircleAvatar(
                  radius: 28,
                  child: signedIn && session.initials.isNotEmpty
                ...
                FilledButton.tonalIcon(
                  key: signedIn
                      ? const Key('settings-account-open')
                      : const Key('settings-account-sign-in'),
                  onPressed: onOpenYandex,
                  icon: Icon(signedIn ? Icons.open_in_new : Icons.login),
                  label: Text(
                    signedIn ? l10n.openYandexMusic : l10n.signIn,
                  ),
                ),
              ],
            );
