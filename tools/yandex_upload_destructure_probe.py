"""Source-free V13 probe for Xc/RG destructuring and parameter bindings.

V12 found a stable local owner ``u`` for the UgcUploadHttpClient anchor in both
provider chunks, while Xc/RG were not connected through ordinary assignments.
V13 therefore inspects only binding forms that V11/V12 intentionally skipped:
object destructuring, array destructuring, and function parameter aliases.

Only safe identifier names and allowlisted upload anchors are emitted. No source
contexts, ordinary string values, credentials, headers, raw ASAR bytes, audio,
or network traffic are included.
"""
...
def _rhs_summary(rhs: str) -> dict[str, Any]:
    value = rhs.lstrip()
    identifiers: list[str] = []
    for match in re.finditer(r"[A-Za-z_$][A-Za-z0-9_$]{0,80}", value[:1200]):
        item = _safe_identifier(match.group(0))
        if not item or item in {"const", "let", "var", "class", "function", "new", "return", "this", "true", "false", "null", "undefined"}:
            continue
    ...
    if value.startswith("new "):
        kind = "new-expression"
    elif re.match(r"[A-Za-z_$][A-Za-z0-9_$]*(?:\.[A-Za-z_$][A-Za-z0-9_$]*)*\s*\(", value):
        ...
        kind = "other-expression"
    return {
        "kind": kind,
        "identifiers": identifiers,
        "anchors": [anchor for anchor in ROLE_ANCHORS if anchor in value],
    }


def _strip_default(value: str) -> str:
    parts = contract_probe._split_top_level(value)  # noqa: SLF001
...
def _object_bindings(body: str, targets: set[str]) -> list[dict[str, Any]]:
    ...
    for match in _OBJECT_BINDING_RE.finditer(body):
        ...
        rhs = _rhs_summary(body[match.end():end])
        results.append({"kind": "object-destructure", "pairs": pairs, "rhs": rhs})
    return results[:80]
...
def build_report(
    ...
    *,
    ...
    return {
        "format": "musicark-yandex-upload-destructure-report-v1",
        "source": "asar-targeted-destructure-static-scan",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Resolve Xc/RG destructuring/parameter bindings without source output.")
...
def main() -> int:
    ...
    print(f"Wrote sanitized V13 destructure report: {args.output}")
    return 0
